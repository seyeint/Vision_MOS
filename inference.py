...
def extract_frames(video_path, output_folder):
    """Extract frame images from video to output folder."""
...
def extract_flow(rgb_path, flow_output_path):
    """ Calculate optical flow for the frames in the input folder using RAFT.
        Saves result into output folder."""
    gap = [1, 2]
    reverse = [0, 1]
    batch_size = 4
...
def inference(args):
    """
    Change the configuration path to appropriate path if necessary.
    Install segment-anything is necessary.
    """
    ...
    os.system("python evaluation.py "
              ...
              "--dataset example --flow_output {} --img_output {} --name {} --max_obj 20 --save_path {}".format(
...
if __name__ == '__main__':
    ...
    if args.visualize_flow:
        ...
        create_video_from_images(flow_path, f'{args.flow_output_path}/flow.mp4', fps=24)

    if args.run_flowsam:
        inference(args=args)

    if args.visualize_output:
        output_path = os.path.join(args.flowsam_output_path, f'nonhung/output/images/{filename}')
        create_video_from_images(output_path, f'output/{filename}.mp4', fps=24)
